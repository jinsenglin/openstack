...
# create subnet
	export SUBNET_ID=$(neutron subnet-create $NETWORK_ID 192.168.100.0/24 --name $NETWORK_NAME --dns-nameserver 8.8.8.8 -f shell --variable id | tail -n 1 | awk 'BEGIN {FS="="}; {print $2}' | sed 's/"//g')
