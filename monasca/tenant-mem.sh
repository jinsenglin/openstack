...
NOW=$(date +%s)
START=$(date -r $(( $NOW - 28890 )) +%Y-%m-%dT%H:%M:%SZ); END=$(date -r $(( $NOW - 28860 )) +%Y-%m-%dT%H:%M:%SZ);

function query() {
monasca measurement-list --dimensions tenant_id=$TENANT --endtime $END --merge_metrics vm.mem.total_mb $START
}
